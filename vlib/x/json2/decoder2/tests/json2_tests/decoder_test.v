...
fn test_raw_decode_string() {
	str := decoder2.decode[json2.Any]('"Hello!"')!
	assert str.str() == 'Hello!'
}

fn test_raw_decode_string_escape() {
	jstr := decoder2.decode[json2.Any]('"\u001b"')!
	str := jstr.str()
	assert str.len == 1
	assert str[0] == 27
}
...
fn test_raw_decode_number() {
	num := decoder2.decode[json2.Any]('123')!
	assert num.int() == 123
}

fn test_raw_decode_array() {
	raw_arr := decoder2.decode[json2.Any]('["Foo", 1]')!
	arr := raw_arr.arr()
...
fn test_raw_decode_bool() {
	bol := decoder2.decode[json2.Any]('false')!
	assert bol.bool() == false
}

fn test_raw_decode_map() {
	raw_mp := decoder2.decode[json2.Any]('{"name":"Bob","age":20}')!
	mp := raw_mp.as_map()
...
fn test_raw_decode_string_with_dollarsign() {
	str := decoder2.decode[json2.Any](r'"Hello $world"')!
	assert str.str() == r'Hello $world'
}

fn test_raw_decode_map_with_whitespaces() {
	raw_mp := decoder2.decode[json2.Any](' \n\t{"name":"Bob","age":20}\n\t')!
	mp := raw_mp.as_map()
	assert mp['name'] or { 0 }.str() == 'Bob'
	assert mp['age'] or { 0 }.int() == 20
}
...
fn test_raw_decode_map_invalid() {
	decoder2.decode[json2.Any]('{"name","Bob","age":20}') or {
...
fn test_raw_decode_array_invalid() {
	decoder2.decode[json2.Any]('["Foo", 1,}') or {
