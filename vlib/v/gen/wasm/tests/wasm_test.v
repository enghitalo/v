...
// Tests that should be skipped from native/wasm comparison
...
fn test_wasm() {
	...
	for test in tests {
		...
		// Test without -b wasm (native compilation) - skip for tests in skip list
		bench.step()
		if skip_comparison {
			eprintln(bench.step_message_ok('${relative_test_path} (native) - SKIPPED'))
		} else {
			cmd_native := '${os.quoted_path(vexe)} run ${os.quoted_path(full_test_path)} 2> ${os.quoted_path(tmperrfile)}'
			if is_verbose {
				println(cmd_native)
			}
			res_native := os.execute(cmd_native)
			if res_native.exit_code != 0 {
				eprintln('failed native: ${full_test_path}')
				bench.fail()
				eprintln(bench.step_message_fail(cmd_native))
				...
				if expected_ := os.read_file(outfile) {
					mut expected := expected_
					expected = expected.trim_right('\r\n').replace('\r\n', '\n')
					mut found := res_native.output.trim_right('\r\n').replace('\r\n',
						'\n')
					found = found.trim_space()
					if expected != found {
						println(term.red('FAIL (native)'))
						println('============')
						println('expected: "${expected}" len=${expected.len}')
						println('============')
						println('found:"${found}" len=${found.len}')
						...
						bench.ok()
						eprintln(bench.step_message_ok('${relative_test_path} (native)'))
					}
				} else {
					bench.ok()
					eprintln(bench.step_message_ok('${relative_test_path} (native)'))
